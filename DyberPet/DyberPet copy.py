...
class PetWidget(QWidget):
    ...
    def _set_menu(self, pets=()):
        ...
        # Add new menu for opening olight.com
        # self.web_menu = RoundMenu(self.tr("Visit Website"))
        # self.web_menu.setIcon(QIcon(os.path.join(basedir,'res/icons/website.svg')))
        # web_action = _build_act("Olight Website", self.web_menu, self.open_web, url="https://www.olight.com")
        # self.web_menu.addAction(web_action)

        # self.web_menu = RoundMenu(self.tr("召唤AI智能体"))
        # visit_website_action = Action(QIcon(os.path.join(basedir,'res/icons/website.svg')), "去访问", self)
...
def _build_act(name: str, parent: QObject, act_func, url=None, icon=None) -> Action:
    """
    构建改变菜单动作
